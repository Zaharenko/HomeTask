...
var mqls = {
	...
	'xs': window.matchMedia("(max-width: 419px)")
};
...
(function ($) {
	...
	$(document).ready(function () {
		...
		if (!mqls.md.matches) new WOW({ offset: 0 }).init();

		// Загрузка подарка
		var getGift = function getGift() {
			try {
				var gift_id = localStorage.getItem('gift_id');
				if (typeof gift_id == 'undefined' || gift_id == null) gift_id = 0;
			} catch (e) {
				gift_id = 0;
			}

			ajaxRequest({ 'method': 'gift', 'gift_id': gift_id }, function (json) {
				// Установка активного поадрка в модальном окне
				$('.gift-slider').find('.item-slider[data-gift-id="' + json.data.response.id + '"]').addClass('active');
				var $gift = $('.gift:first');
				$gift.removeClass('gift_loading').findElement('name').html(json.data.response.name);
				$gift.findElement('image').find('img').attr('src', '').addClass('lazyload').attr('data-src', json.data.response.short_image);
			});
		};
		...
		// Эффект парллакса
		if ($('.rellax').length) var rellax = new Rellax('.rellax');

		// Слайдер для подзаголовка
		...
		var $production_slider = $('.production-slider');

		// Функция для подстановки текста к стрелкам
		var setArrowSlick = function setArrowSlick(slick, current_slide) {
			if ($(slick.$slides[current_slide - 1]).length) {
				$(slick.$slider).find('.slick-navigation_prev').find('.slick-navigation__text').html($(slick.$slides[current_slide - 1]).data('slider-description'));
			}

			if ($(slick.$slides[current_slide + 1]).length) {
				$(slick.$slider).find('.slick-navigation_next').find('.slick-navigation__text').html($(slick.$slides[current_slide + 1]).data('slider-description'));
			}
		};
		// На инициализацию
		$production_slider.on('init', function (event, slick) {
			setArrowSlick(slick, 0);
		});
		// На изменение
		$production_slider.on('beforeChange', function (event, slick, currentSlide, nextSlide) {
			setArrowSlick(slick, nextSlide);
			$('#work').find('.title-section').findElement('arrow').fadeOut();
		});

		$production_slider.on('click', '.next-slide', function () {
			$production_slider.slick('slickNext');
		});
		...
		$production_slider.slick({
			...
			infinite: false,
			prevArrow: '<div class="slick-navigation slick-navigation_prev"><div class="slick-navigation__arrow"></div><div class="slick-navigation__text"></div></div>',
			nextArrow: '<div class="slick-navigation slick-navigation_next"><div class="slick-navigation__arrow"></div><div class="slick-navigation__text"></div></div>',
			fade: true,
			swipe: false,
			touchMove: false
		});
		...
		// Выбор подарка
		$(document).on('click', '.gift-slider .item-slider', function (event) {
			event.preventDefault();
			$(this).addClass('active').siblings().removeClass('active');
			$('.gift-choise').findElement('name').find('span').text($(this).data('gift-name'));
			localStorage.setItem('gift_id', $(this).data('gift-id'));
		});
		...
		// Основная часть с КВИЗом


		// Обработчик для модального окна с подарками
		$(document).on('click', '#modal-choice-gift .gift-slider .item-slider', function (event) {
			$('.gift:first').addClass('gift_loading');
			$.fancybox.close();
			getGift();
		});

		// Заметки для КВИЗа
		$(document).on('click qtip:show', '.note', function (event_on) {
			var note_hard = 'note-tooltip_hard';
			var $tooltip_note = $(this).qtip({
				prerender: false,
				position: {
					my: 'center left',
					at: 'center left',
					target: 'event',
					container: $('.notes').findElement('wrapper'),
					viewport: $('.notes').findElement('viewport'),
					adjust: {
						// y: -7,
						method: 'flipinvert',
						resize: false
				...
				hide: {
					event: event_on.type != 'qtip:show' ? 'unfocus' : false,
					// delay: 3000,
					fixed: true
				},
				events: {
					show: function show(event, api) {
						if (event_on.type == 'qtip:show') $(this).addClass(note_hard);
					},
					hide: function hide(event, api) {
						$(this).removeClass(note_hard);
						api.destroy(true);
					}
				},
				content: {
					text: function text(event, api) {
						return $(this).find('.tooltip').clone(true, true);
					},
					button: 'Закрыть'
				}
			}, event_on);
		...
		// Слайдер с вопросами
		var $quiz_slider = $('.quiz-slider'),
		    $projects_slider = $('.projects-slider');

		$quiz_slider.on('init', function () {
			checkQuestion();
			checkNote();
		});
		...
		$quiz_slider.slick({
			...
			infinite: false,
			prevArrow: '',
			nextArrow: '<div class="slick-next btn btn_big btn_yellow disabled">На следующий шаг&nbsp;&rarr;</div>',
			fade: true,
			swipe: false,
			touchMove: false,
			dots: true,
			appendDots: $('.step-number').findElement('bar')
		});
		...
		// Вопросов на блок более 1
		$('.accordion').accordion({
			header: ".question__block",
			heightStyle: "content",
			classes: {
				"ui-accordion-header-active": "question__block_active"
			},
			beforeActivate: function beforeActivate(event, ui) {
				$('.note').qtip('hide');
				var answer = $(ui.oldPanel[0]).find('input[type="radio"]:checked, input[type="checkbox"]:checked').siblings('label').text();
				$(ui.oldHeader[0]).findElement('answer').text(answer);
			},
		...
		function checkQuestion() {
			var $current_gquestions = $quiz_slider.find('.slick-current'),
			    next_gquestion = false;
			if ($current_gquestions.find('input[type="radio"], input[type="checkbox"]').length > 0) {
				$current_gquestions.find('fieldset[data-type="radio"], fieldset[data-type="checkbox"]').each(function (index) {
					if ($(this).find('input:checked').length > 0) {
						$(this).next().removeClass('disabled').parents('.accordion').accordion("option", "active", index + 1);
			...
			if (next_gquestion) {
				$quiz_slider.find('.slick-next').addClass('disabled').removeClass('btn_radiance');
			} else {
				$quiz_slider.find('.slick-next').removeClass('disabled').addClass('btn_radiance');
			}
		}

		function checkNote() {
			$('.note').qtip('hide');
			var note_id = $quiz_slider.find('.slick-current .question__block_active:first').parents('fieldset').data('note-id'),
			    $question_note = $('.note[data-note-id="' + note_id + '"]');
			if ($question_note.length) $question_note.show().trigger('qtip:show');
		}

		// Обработчики на ответы по вопросам
		$quiz_slider.on('afterChange', function (event, slick, currentSlide) {
			checkQuestion();
			checkNote();

			$('.step-number').findElement('text').find('span').text(currentSlide + 1);
			$('.note-technology').removeClass('active');
			// Выполнение цели
			try {
				yaCounter45637056.reachGoal('SMARTO_Q' + currentSlide);
				// console.log('SMARTO_Q' + currentSlide);
			} catch (e) {};
		});

		$(document).on('change', '.slick-current input[type="radio"], .slick-current input[type="checkbox"]', function () {
			checkQuestion();
		});

		// Для смешанного типа ответа radio/text
		$(document).on('change', '.slick-current .magic-text input[type="text"]', function () {
			$(this).parents('.answer-group').find('input[type="radio"], input[type="checkbox"]').val($(this).val());
		});

		$(document).on('click', '.magic-text input[type="text"]', function () {
			$(this).parents('.answer-group').find('input[type="radio"], input[type="checkbox"]').prop('checked', true).trigger('change');
		});

		// Вопросы тип ползунок
		function setRangeSlider($slider, count_range) {
			count_range = count_range || 4;
			var slider_min = $slider.slider("option", "min"),
			    slider_max = $slider.slider("option", "max"),
			    interval = (slider_max - slider_min) / count_range,
			    $range = $slider.parents('.slider').findElement('range');

			$range.css({
				width: 100 + 100 / count_range + '%',
				left: -100 / count_range / 2 + '%'
			}).empty();

			for (var i = 0; i <= count_range; i++) {
				var mode = Math.ceil(slider_min).toString().length,
				    interval_text = '',
				    suffix = '',
				    range = 1;
				...
				var interval_value = slider_min / range,
				    n = parseFloat((interval_value - Math.floor(interval_value)).toPrecision(1));

				interval_value = parseInt(interval_value) + (n > 0.2 ? n : 0);
				interval_text = interval_value + ' ' + suffix;

				$range.append('<div class="item-range" style="width: ' + 100 / count_range + '%;" data-value="' + interval_value * range + '"><span>' + interval_text + '</span></div>');
				slider_min += interval;
			}
		}

		$('.slider-range-group').each(function () {
			var $obj = $(this),
			    slider_min = $obj.data('min'),
			    slider_max = $obj.data('max'),
			    slider_step = $obj.data('step'),
			    slider_value = $obj.data('value');

			// Инициализация слайдера
			var $slider = $obj.find('.slider').findElement('bar');
			$slider.slider({
				animate: "slow",
				range: "min",
				min: slider_min,
			...
			var mask_opts = {
				...
				showMaskOnHover: false,
				onKeyDown: function onKeyDown(event, buffer, caretPos, opts) {
					if (event.originalEvent.keyCode == 13) event.preventDefault();
					return true;
				}
			};

			if ($obj.hasClass('currency')) {
				mask_opts['suffix'] = ' $';
			} else if ($obj.hasClass('square')) {
				mask_opts['suffix'] = ' м2';
			}

			var $input = $obj.find('input').val($slider.slider("value")).inputmask(mask_opts);
		});

		$(document).on('change', '.slider-range-group input', function () {
			$(this).siblings('.slider').findElement('bar').slider("value", $(this).inputmask('unmaskedvalue'));
		});

		$(document).on('click', '.item-range span', function () {
			$(this).parents('.magick-slider').findElement('bar').slider('value', $(this).parents('.item-range').data('value'));
		});

		// Смена зафиксированного тултипа

		$(document).on('mouseenter', '.answer-group[data-note-id]', function () {
			var note_id = $(this).data('note-id'),
			    $tooltip = $('.note[data-note-id="' + note_id + '"]').find('.tooltip');
			if ($tooltip.length) {
				var api = $('.note-tooltip_hard').qtip('api');
				if (typeof api != 'undefined' && api != null) api.set('content.text', $tooltip.clone(true, true));
			}
		});

		$(document).on('mouseleave', '.answers', function () {
			var note_id = null,
			    $answer_checked = $(this).find('input[type="radio"]:checked, input[type="checkbox"]:checked:first');
			// Получаем ID выбранного тултипа
			if ($answer_checked.length) {
				note_id = $answer_checked.parents('.answer-group').data('note-id');
			} else {
				note_id = $(this).parents('.question').data('note-id');
			}

			var $tooltip = $('.note[data-note-id="' + note_id + '"]').find('.tooltip');
			if ($tooltip.length) {
				var api = $('.note-tooltip_hard').qtip('api');
				if (typeof api != 'undefined' && api != null) api.set('content.text', $tooltip.clone(true, true));
			}
		...
		// Отладка координат
		$(document).on('click', '.debug-coord', function (e) {
			var posX = $(this).offset().left,
			    posY = $(this).offset().top;
			console.log('x: ' + (e.pageX - posX - 14) * 100 / $(this).outerWidth() + ', y: ' + (e.pageY - posY - 14) * 100 / $(this).outerHeight());
		});

		// Последний слайд
		$quiz_slider.on('afterChange', function (event, slick, currentSlide) {
			if (slick.currentSlide >= slick.slideCount - slick.options.slidesToShow) {
				// Включаем последний слайд
				$('.project').findElement('last-slide').fadeIn();
				$('.notes').hide();
				var $build_list = $('.build').findElement('list').find('li');
				$build_list.each(function (index) {
					(function (element, i) {
						var t = setTimeout(function () {
							$(element).animateCss('fadeInUp').show().prev().remove();
							if ($build_list.length == i + 1) {
								$('.build').addClass('build_success').findElement('arrow').fadeIn();
								$('.quiz').findElement('gifts').animateCss('pulse').addClass('radiance');
							}
						}, 1800 * i);
					})(this, index);
				});
		...
		// Уникальные условия для вопросов

		// Выбор проекта дома
		$('fieldset[data-code="target"]').on('change', 'input', function (event, elements) {
			$(event.delegateTarget).addClass('disabled');
			var $btn = $quiz_slider.find('.slick-next');

			

			$('.project').findElement('example').fadeIn();

			$projects_slider.slick('slickGoTo', $(this).data('value'));
		});

		$projects_slider.on('afterChange', function () {
			$('fieldset[data-code="target"]').removeClass('disabled');
		});

		$('fieldset[data-code="target"]').on('change', 'input', $.debounce(500, function (event) {
			var current_project = $(this).data('value') - 1;
			if (typeof current_project == 'undefined' || current_project == null) current_project = 0;

			ajaxRequest({ 'method': 'coord' }, function (json) {
				$('.note-technology').css('visibility', 'visible').addClass('active');
				$('#modal-technology').find('.technology').findElement('item').eq(json.data.response[current_project].technology).show().siblings().hide();

				$quiz_slider.find('.slick-next').find('.loader').remove();
				$.each(json.data.response[current_project].coord, function (note_id, coord) {
					$('.note[data-note-id="' + note_id + '"]').css({
						"left": coord[0] + "%",
						"top": coord[1] + "%"
					});
				});
		...
		$('fieldset[data-code="square"]').find('.slider').on('slidechange', function (event, ui) {
			$('.project').findElement('square-value').text(ui.value);
		});

		$quiz_slider.on('afterChange', function (event, slick, currentSlide) {
			var $square = $quiz_slider.find('.slick-current').find('fieldset[data-code="square"]');
			if ($square.length) $('.project').findElement('square-value').text($square.find('.slider').findElement('bar').slider('value'));
		});

		$quiz_slider.on('afterChange', function (event, slick, currentSlide) {
			var $slider = $quiz_slider.find('.slick-current .magick-slider');
			if ($slider.length) {
				$slider.each(function () {
					var $current_slider = $(this).find('.slider').findElement('bar'),
					    value = $current_slider.data('slider-value');
					$current_slider.slider("value", value).slider("option", "animate", false);
				});
			}
		});

		// Технология строительства
		$(document).on('click', '.note-technology', function () {
		...
		$('.form-std').ajaxForm({
			...
			success: function success(responseText, statusText, xhr, $form) {
				...
				try {
					// console.log(goal_form);
					yaCounter45637056.reachGoal(goal_form);
				} catch (e) {};
...
// function onChangeiOS(element)
// {
// 	if (navigator.userAgent.search(/Safari/) > -1) {
// 		alert("*");
// 		element.trigger('changeios');
// 	}
// }
